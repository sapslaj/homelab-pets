...
// Change the type in the entry
func (e *Entry) SetType(v []byte) error {
	if e.isControl {
		return errors.New("control entry does not have a class")
	}
	if len(v) != 0 && !dns_types_lut[string(v)] {
	...
	classIs := e.find(useClass)
	if len(classIs) == 1 {
		e.tokens = append(
			e.tokens[:classIs[0]+1],
			append(
				[]taggedToken{tttSpace, tType},
				e.tokens[classIs[0]+1:]...
			)...
		)
		return nil
	}
	return e.addAfterDomain(tType)
}
...
// Remove an entry from the zonefile
func (z *Zonefile) RemoveEntry(e Entry) {
	z.entries = slices.DeleteFunc(z.entries, func(se Entry) bool {
		return e.String() == se.String()
	})
}
...
// tagged token template type
var tttType taggedToken = taggedToken{
	token{val: []byte{'.'}, typ: tokenItem}, useType}
